...
from ....api.pydantic.schemas import Pagination

from .dependencies import get_link_repo
from .schemas import LinkSchema, LinkCreateSchema, LinkListSchema, LinkUpdateSchema

from ....domain.link.repository import AbstractLinkRepository
from ....domain.link.schemas import Link, LinkCreateDTO, LinkUpdateDTO
...
def create_short_link(
    ...
    repo: AbstractLinkRepository = Depends(get_link_repo),
) -> JSONResponse:
    ...
    return JSONResponse(content=data.model_dump(mode="json"), status_code=status.HTTP_201_CREATED)


@router.get("/details/{url_id}", response_model=LinkSchema)
def get_link_data(url_id: UUID, repo: AbstractLinkRepository = Depends(get_link_repo),) -> JSONResponse:
    try:
        link = repo.get(url_id)
