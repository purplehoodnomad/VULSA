...
class PostgresLoginUseCase(AbstractLoginUseCase):
    ...
    async def execute(
        ...
        async with self.uow as uow:
            user_entity = await uow.user_repo.get_by_email(Email(dto.email)) # type: ignore

            if not user_entity.check_password(dto.password):
                raise PasswordMismatchException(user_entity.user_id.value)
            
            token_entity = await uow.token_repo.get_latest_for_user(user_entity.user_id) # type: ignore
            if token_entity is not None:
                token_entity.drop()
                dropped_token = await uow.token_repo.update(token_entity) # type: ignore
            
            created_token = Token.create(user_id=user_entity.user_id)
            new_token_entity = await uow.token_repo.create(entity=created_token) # type: ignore
