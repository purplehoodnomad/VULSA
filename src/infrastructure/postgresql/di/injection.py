...
def build_link_uow(session: AsyncSession = Depends(get_async_session)) -> AbstractLinkUnitOfWork:
    return Container.link_uow_factory(session=session)


def build_user_uow(session: AsyncSession = Depends(get_async_session)) -> AbstractUserUnitOfWork:
    return Container.user_uow_factory(session=session)


def build_auth_uow(session: AsyncSession = Depends(get_async_session)) -> AbstractAuthUnitOfWork:
